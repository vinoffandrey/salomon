...
export function pageNavigation() {
	...
	function pageNavigationAction(e) {
		if (e.type === "click") {
			...
			if (targetElement.closest("[data-goto]")) {
				...
				} else if (gotoLink.tagName === "A") {
					let link = gotoLink.getAttribute("href") + `${hash}`;
					gotoLink.setAttribute("href", link);
					link = gotoLink.getAttribute("href");
	...
	if (getHash()) {
		...
		window.addEventListener(
			...
			function () {
				nav = nav.split("#");
				const link = document.querySelector(`a[href*="#${nav[0]}"]`);
				if (link && link.href !== "#") {
					setTimeout(function () {
						link.click();
					}, 1000);
